...
class Koan10 extends GroovyTestCase {
	...
	void test01_XmlSlurpersReader() {
		...
		def movieCatalogElements = new XmlSlurper().parse(new File("${prefix}movies.xml"))
		movieCount = movieCatalogElements.movie.size()
		// ------------ STOP EDITING HERE  ----------------------
		assert movieCount == 7
		...
		// ------------ START EDITING HERE ----------------------
		def nodes = movieCatalogElements.movie.findAll { it.title.text().toLowerCase() =~ "the" }
		moviesWithThe = nodes.collect { it.title.text() }
		// ------------ STOP EDITING HERE  ----------------------
		...
		// ------------ START EDITING HERE ----------------------
		movieIdsGreaterThan5 = movieCatalogElements.movie.findAll { it.@id.text().toInteger() > 5 }.size()
		// ------------ STOP EDITING HERE  ----------------------
	...
	void test02_XmlSlurpersReader2() {
		...
		def prefix = 'src/test/groovy/org/groovykoans/koan10/'
		def movieCatalogElements = new XmlSlurper().parse(new File("${prefix}movies.xml"))
		sortedList = movieCatalogElements.movie.list().sort { movie1, movie2 ->
			def year1 = movie1.year.text().toInteger()
			def year2 = movie2.year.text().toInteger()
			return (year1 == year2) ? movie1.title.text() <=> movie2.title.text() : year1 <=> year2
	...
	void test03_XmlMarkupBuilder1() {
		...
		// ------------ STOP EDITING HERE  ----------------------
		assert formatXml(writer.toString()) == formatXml("<html><body><h1>title</h1></body></html>")
	}
	...
	void test04_XmlMarkupBuilder2() {
		...
		def convertMovieCatalog = {
		}

		markupBuilder.movies {
			movieCatalogElements.movie.each{
				//convertMovieCatalog(it)
				movieNode ->
					def movieId = movieNode.@id.text()
					def movieTitle = movieNode.title.text()
					def movieYear = movieNode.year.text()

					movie(id:"${movieId}", title:"${movieTitle}", year:"${movieYear}")
			}
		}

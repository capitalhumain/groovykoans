...
class Koan11 extends GroovyTestCase {
	...
	void test02_MoreOnInsertingData() {
		...
		Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
			...
			// ------------ START EDITING HERE ----------------------
			def insertQuery = "insert into PERSON(FIRSTNAME, LASTNAME) values (?, ?)"


			def prefix = 'src/test/groovy/org/groovykoans/koan11/'
			def file = new File("${prefix}cast.txt")

			file.readLines().each { line ->
				db.execute(insertQuery, line.split())
			}
			...
			// ------------ START EDITING HERE ----------------------
			def cast2Elements = new XmlSlurper().parse(new File("${prefix}cast2.xml"))

			cast2Elements.character.@actor.each{ actor ->
				db.execute(insertQuery, actor.text().split())
			}
			// ------------ STOP EDITING HERE  ----------------------
			...
			// ------------ START EDITING HERE ----------------------
			person.add(firstname:"Johny", lastname:"Doe")
			// ------------ STOP EDITING HERE  ----------------------
	...
	void test03_IteratingResults() {
		Sql.withInstance('jdbc:h2:mem:groovyDb3', 'sa', '', 'org.h2.Driver') { db ->
			...
			def lastNameRose
			// ------------ START EDITING HERE ----------------------
			lastNameRose = db.firstRow('select LASTNAME from Person where FIRSTNAME = :firstName', [firstName:'Rose']).lastname
			// ------------ STOP EDITING HERE  ----------------------
			...
			def eCount = 0
			// ------------ START EDITING HERE ----------------------
			db.eachRow("select * from person"){ person ->
				eCount += person.lastname.count('e')
			}
			// ------------ STOP EDITING HERE  ----------------------
			assert eCount == 2
		}
	...
	void test04_UpdatingTables() {
		Sql.withInstance('jdbc:h2:mem:groovyDb4', 'sa', '', 'org.h2.Driver') { db ->
			...
			// ------------ START EDITING HERE ----------------------
			db.eachRow("select * from person"){person ->
				person.firstname = person.firstname.contains('a') ? 'Alf' : person.firstname
			}
			// ------------ STOP EDITING HERE  ----------------------
